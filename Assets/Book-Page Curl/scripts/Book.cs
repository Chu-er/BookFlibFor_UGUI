...
public class Book : MonoBehaviour {
    ...
    /// <summary>
    /// 作用是将一个世界坐标转化为相对于书的本地坐标
    /// </summary>
    /// <param name="global"> 世界坐标</param>
    /// <returns></returns>
    ...
    void Update()
    {
        if (pageDragging&&interactable) //交互的
        {
            UpdateBook();
        }
    ...
    public void UpdateBookRTLToPoint(Vector3 followLocation)
    {
        ...
        f = followLocation;
        Shadow.transform.SetParent(ClippingPlane.transform);
        Shadow.transform.localPosition = new Vector3(0, 0, 0);
        Shadow.transform.localEulerAngles = new Vector3(0, 0, 0);
        Right.transform.SetParent(ClippingPlane.transform);
    ...
    /// On Drag CallMethed
    /// </summary>
    /// <param name="point"> 鼠标的屏幕坐标</param>
    public void DragRightPageToPoint(Vector3 point)
    {
        ...
        //设置两个mask 的pivot 中心点  旋转缩放是根据它来的  这样才游戏中看起来两个点是重合的
        NextPageClip.rectTransform.pivot = new Vector2(0, 0.12f);
        ClippingPlane.rectTransform.pivot = new Vector2(1, 0.35f);


        //left  显示 并且与rightNext 重合
        Left.gameObject.SetActive(true);
        Left.rectTransform.pivot = new Vector2(0, 0); //左下角设置为pivot
